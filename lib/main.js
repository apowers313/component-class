...
module.exports = class Component {
    ...
    addSetterGetterFeature(name, type) {
        var checkFn;
        var featureSetterGetter = (val) => {
            checkFn(val);
        ...
        function checkBuiltin(t, val) {
            if (typeof val !== t) {
                throw new Error(`feature '${name}' expected type ${t}, got: ` + typeof val);
            }
        }
        ...
        switch (typeof type) {
            ...
            case "object" && type !== null:
                checkFn = checkObject.bind(type);
                break;
            default:
                throw new TypeError("addSetterGetterFeature bad type: " + type);
        }

        this.addFeature(name, featureSetterGetter);
    }
